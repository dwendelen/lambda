...
data class VariableTypeSymbol(val depth: Int): TypeSymbol()
...
//            }
//
//        val (type, _) = ctx.type().accept(TypeVisitor(tableWithParams, 0, false))
//
//        return Pair(table, typeTable + (name to ExternalTypeSymbol(type, ids.size - 1)))
    ...
    override fun visitAssignment(ctx: LambdaParser.AssignmentContext): Pair<SymbolTable, TypeTable> {
        ...
        //val (type, _) = ctx.type().accept(TypeVisitor(typeTable, 0, false))
        //validateType(newExpr, type)
...
class TypeVisitor(
    ...
    private val allowNewType: Boolean
    ...
    override fun visitTypeRef(ctx: LambdaParser.TypeRefContext): Pair<Type, TypeTable> {
        ...
        val (type, table1) = if(foundType == null) {
            if (allowNewType) {
                Pair(InternalTypeSymbol(NewType()), typeTable + ( name to VariableTypeSymbol(depth)))
            } else {
        ...
        return when (type) {
            ...
            is VariableTypeSymbol -> {
                Pair(ClosureType(depth - type.depth - 1), table1)
            }
            is ExternalTypeSymbol -> {
                ...
                val params = paramNames
                    .fold(emptyList<Type>()) { params, p ->
                        val (typ, _) = p.accept(TypeVisitor(typeTable, depth, false))
                        params + typ
                    }
    ...
    override fun visitLambdaType(ctx: LambdaParser.LambdaTypeContext): Pair<Type, TypeTable> {
        val (variable, table1) = ctx.lambdaVarType().accept(TypeVisitor(typeTable, depth, true))
        val (to, table2) = ctx.type().accept(TypeVisitor(table1, depth + 1, false))
